...
def get_ips_from_target(target_str):
    ...
    try:
        # إذا كان المدخل يحتوي على '/' فهذا يعني أنه Subnet (مثل 192.168.1.0/24)
        if '/' in target_str:
            network = ipaddress.ip_network(target_str, strict=False)
            # نستخرج كل الأجهزة المتاحة في الشبكة (نستثني عنوان الشبكة والبرودكاست)
...
def parse_arguments():
    ...
    # إضافة الـ Flags المطلوبة في المشروع (ستكون جاهزة لزملائك لربطها بالفحوصات لاحقاً)
...
def main():
    ...
    # 1. تحليل الهدف وتحويله إلى قائمة IPs
    ...
    # 3. تشغيل الفحص (نمرر القائمة كاملة بدلاً من IP واحد)
    try:
